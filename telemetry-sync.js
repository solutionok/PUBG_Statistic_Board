...
const matcherPcRegionPC = /\.(training|tdm|pc\-).*\.(as|eu|jp|kakao|krjp|na|oc|ru|sa|sea|tournament)\./;
...
async function matchesOfPlatform(platform) {
  try{
    ...
    return await new Promise((resolve, reject) => {
      axioPubgApi.get('/shards/' + platform + '/samples?filter[createdAt-start]=2020-02-07T00:00:00Z')
        .then(r => {
      ...
      axios.get(assetUrl)
        .then(r => {
          if (r.data && r.data) {
            ...
            if (!regionMatches) {
              console.log('----Unknown region detected - ', r.data[0].MatchId)
              resolve('')
            } else {
  
              if (Object.keys(REGIONS).indexOf(regionMatches[1]) !== -1) {
                // console.log('region match possition 1', r.data[0].MatchId)
                resolve(regionMatches[1])
              } else if (regionMatches.length >= 2 && Object.keys(REGIONS).indexOf(regionMatches[2]) !== -1) {
  
                // console.log('region match possition 2', r.data[0].MatchId)
...
async function cacheMatchesOfWinners(topPlayers, platform, logid, cachedUUID){
  ...
  for(let tp in topPlayers){
    ...
    for(let pm in topPlayers[tp]['relationships']['matches']['data']){
      ...
      if(!matchinfo) {
        console.log('failed to fetch the match', matchid)
        continue;
      }
      ...
      if(insertID){
        console.log('UUID: ' + matchid + ' Created ', insertID, 'From player ', topPlayers[tp].id);
        matchids.push(matchid);
      }else{
        console.log('UUID: ' + matchid + ' an issue while cache ', 'From player ', topPlayers[tp].id);
      }
    }
...
async function pulldown() {
  ...
  for (let _p in platforms) {
    ...
    console.log('platform-region : ' + platforms[_p] + ', total matches : ' + matches.length + ', endpoint url : ' + '/shards/' + platforms[_p] + '/samples')

    //match level loop
    for (let _m in matches) {
      if(cachedUUID.indexOf(matches[_m].id)!==-1) {
        console.log('skip for uuid ', matches[_m].id)
        continue;
      }
      ...
      if(!matchDetails) {
        console.log('failed to fetch the match', matches[_m].id)
        continue;
      }
      ...
      if(insertID){
        console.log('UUID: ' + matches[_m].id + ' Created ', insertID);
        ...
        console.log('UUID: ' + matches[_m].id + ' an issue while cache');
      }
